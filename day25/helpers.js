 ...
 * @param {'"x;y"' | "{x,y}" | "list[-y] === [x1,x2,x3,...]"} [inputBag.format]
 * @param {boolean} [inputBag.yAxisGoesDown]
 *
 * @returns {string[]}
 *    e.g. "#..."
 *         "..#."
 *         ".S.."
 */
function paintCoordinates({ list, list2, format = '"x;y"', yAxisGoesDown = false }) {
  let coordinates;
  let coordinates2 = [];
  switch (format) {
    case '"x;y"':
  ...
  for (
    let posY = yAxisGoesDown ? minY : maxY;
    (yAxisGoesDown && posY <= maxY) || (!yAxisGoesDown && posY >= minY);
    posY += yAxisGoesDown ? 1 : -1
  ) {
    let currLine = "";
...
class Progress {
  ...
  step(currStepCount = null) {
    if (!this.handleLogEvent || !this.startTS || currStepCount === 0) {
      return this;
    }

    this.currStepCount = currStepCount == null ? this.currStepCount + 1 : currStepCount;

    const now = Date.now();
    const currProgress = this.currStepCount / this.maxStepCount;
    ...
    const estimatedStepTimeMS = runningMS / this.currStepCount;
    ...
    const secondsPerStep = Math.floor(estimatedStepTimeMS / 100) / 10;
  ...
  finalize(currStepCount = null) {
    ...
    this.handleLogEvent(
      [
        ...
        "finished,",
        this.currStepCount === this.maxStepCount
          ? `processed ${this.maxStepCount} steps`
          : `processed ${this.currStepCount} of ${this.maxStepCount} steps`,
        `in ${runningTimeText}`,
      ].join(" ")
    );
